...
export function LinkButton(nameButton,funct) {  
    ...
    var button=document.getElement(nameButton);
    if (button)
...
export function InsertIFrame(windowid,url) {
   var domid=document.getElement(windowid);   
   //console.log(domid);
...
export function MonitorDomid(areaid,parentclass,childclass,triggerclass,cbChildChanged) {
    ...
    var domid=document.getElement(areaid);
...
export class DomList {    
    constructor (objectclass,parentdomid) {
     //   console.log(`In constructor of DomList with objectclass=${objectclass}`);
        var list = (parentdomid?parentdomid:document).getElement(objectclass);
        //console.log(list)    
        if (list && list[0]) {
...
export function GetCourseInfo(key) {
    ...
    var course_level_items=document.getElement("course-level-id"); 
